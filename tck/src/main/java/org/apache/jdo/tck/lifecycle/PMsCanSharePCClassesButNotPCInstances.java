 ...
 * <B>Keywords:</B> PM share PC class <br>
 * <B>Assertion IDs:</B> A5.2-1,A5.2-2,A5.2-3 <br>
 * <B>Assertion Description: </B> Several JDO PersistenceManagers might be coresident and might
 * share the same persistence capable classes] (although a JDO instance can be associated with only
 * one PersistenceManager at a time)
 */
public class PMsCanSharePCClassesButNotPCInstances extends JDO_Test {
  ...
  public void testNonSharedPC() {
    ...
    threads = 0;
    attempts = 0;
    // insertedCount = 0; //Don't reset this value since we have not deleted the inserted object
    // from the shared PC test above
  ...
  protected void insertObjects(boolean sharedPC) {
    ...
    for (int i = 0; i < inserters; i++) {
      if (sharedPC) { // each thread shares one PC
        if (pc == null) pc = new StateTransitionObj(i);
    ...
    for (Map.Entry<Thread, Throwable> entry : exceptions) {
      ...
      if (throwable instanceof AssertionFailedError) fail(ASSERTION_FAILED, message);
      else throw new JDOFatalException(message, throwable);
    }
  }
  ...
  class Inserter implements Runnable {
    ...
    protected synchronized void announce(String msg, Object pc) {
      ...
      if (debug) logger.debug(msg + this.getClass().getName() + ": " + oid + ", " + pc);
    }
    ...
    public void run() {
      ...
      try {
        ...
        int objCount = 0;
        if (debug) logger.debug("getting Extent of " + instanceClass.getName());
        tx.begin();
